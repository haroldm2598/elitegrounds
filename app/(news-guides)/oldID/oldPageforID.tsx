		...
		<main className={`${lato.className} bg-custom-main400`}>
			<section className='min-h-screen max-w-7xl mx-auto py-10 px-4 md:px-0'>
				{[dataResult]?.map((item, index) => {
					const midIndex = Math.ceil(item!.descriptions!.length / 2);
					const firstColumn = item!.descriptions?.slice(0, midIndex);
					const secondColumn = item!.descriptions?.slice(midIndex);
